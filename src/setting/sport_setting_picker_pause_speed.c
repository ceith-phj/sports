...
#define to_set(psi)             &psi->ssm.assist.autoPause.bySpeed
...
static void _pickerCheckEvent(picker_screen_info *psi)
{
    ...
    /* 组合数据 */
    set->data = wgt->tool.getNumberByIndex(wgt->index[0], set->min, set->min, set->interval);

    /* 保存设置 */
    psi->ssm.assist.autoPause.standing.onOff = false;
    psi->ssm.assist.autoPause.bySpeed.onOff = true;
    psi->ssm.assist.autoPause.byPace.onOff = false;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_AUTO_PAUSE), sub_title);
...
const picker_ops picker_ops_pause_speed = {
