...
#define SETTING_SCREEN              "SportSettingDrinkScreen"                   /**< 设置所在screen. */
#define SETTING_LAYER               "SportSettingDrinkLayer"                    /**< 设置所在layer. */
...
static void _textEvent(sport_setting_container *ssc, uint16_t index)
{
    ...
    case INDEX_TARGET_TIME:
        switchPickerScreen(PICKER_ID_REMINDER_DRINK_TIME, &ssc->rsi.set, ssc->change);
        break;

    case INDEX_TARGET_DISTANCE:
        switchPickerScreen(PICKER_ID_REMINDER_DRINK_DISTANCE, &ssc->rsi.set, ssc->change);
        break;
...
static void _rlwEvent(radio_list_widget *rlw, lw_event_type event, int index, int lastIndex)
{
    ...
    SportSettingUseWorkoutStatistics *rawSet = &ssc->rsi.set.assist.reminder.drinking.drink;
    ...
    /* 设置选中值 */
    ssc->rsi.set.assist.reminder.drinking.choice = imap[index];
    ssc->change = true;
}
...
static void _settingCreate(sport_setting_container *ssc)
{
    ...
    SportSettingUseWorkoutStatistics *defSet = &ssc->rsi.def.assist.reminder.drinking.drink;
    SportSettingUseWorkoutStatistics *rawSet = &ssc->rsi.set.assist.reminder.drinking.drink;
    lw_set_utils *lsu = &context->lsu;

    /* 创建列表控件 */
    context->rlw = radioList_Create(SETTING_SCREEN, SETTING_LAYER, _rlwEvent);
    radioList_SetListAttr(context->rlw, LW_ATTR_TITLE, (void *)_ST(TK_DRINK));
    ...
    /* 设置 - 消耗 */
    snprintf(value[i], SZ_SET_BUF, _ST(TK_BY_KCAL), rawSet->calories.data);
...
const sport_setting_ops setting_ops_drink = {
