...
#define to_set(psi)             &psi->ssm.assist.target.useWorkoutStatistics.trips
...
static void saveSetting(picker_screen_info *psi)
{
    ...
    rawSet->useWorkoutStatistics.trips.onOff = true;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_TARGET_TRIPS), NULL);
...
const picker_ops picker_ops_target_trips = {
