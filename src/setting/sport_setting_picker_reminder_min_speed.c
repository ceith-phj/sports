...
#define to_set(psi)             &psi->ssm.assist.reminder.min_speed
...
static bool checkVaildData(picker_screen_info *psi)
{
    PickerWidget *wgt = psi->picker;
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.max_speed;
    ...
    {
        set->data = aim->data - 5;
        switchDialogScreen(DLG_ID_MIN_SPEED, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_SLOW_SPEED), sub_title);
...
const picker_ops picker_ops_reminder_min_speed = {
