...
#define to_set(psi)             &psi->ssm.assist.reminder.energy.energy.calories
...
static void _pickerCheckEvent(picker_screen_info *psi)
{
    ...
    /* 保存设置 */
    set->onOff = true;
    psi->ssm.assist.reminder.energy.choice = 2;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_COM_SET_KCAL), NULL);
...
const picker_ops picker_ops_reminder_energy_consume = {
