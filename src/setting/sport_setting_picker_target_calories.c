...
#define to_set(psi)             &psi->ssm.assist.target.useWorkoutStatistics.calories
...
static void saveSetting(picker_screen_info *psi)
{
    ...
    rawSet->useWorkoutStatistics.calories.onOff = true;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_TARGET_CALORIES), NULL);

    /* 设置列 1 */
    setPicker_ItemAttr(psi->picker, &pdata, 0, NULL, _GT(TEXT_ID_KCAL), true, false);
...
const picker_ops picker_ops_target_calories = {
