...
#define to_set(psi)             &psi->ssm.assist.reminder.backTo.back.time
#define getSportTime()          (*(uint32_t *)sportManager()->dataManager()->getRtData(SPORT_RT_ATTR_TYPE_DURATION_NET))
...
static void _pickerCheckEvent(picker_screen_info *psi)
{
    ...
    data_1 = wgt->tool.getNumberByIndex(wgt->index[1], 0, 55, set->interval);
    set->data = (data_0 * 60 + data_1);

    /* 保存设置 */
    set->onOff = true;
    psi->ssm.assist.reminder.backTo.choice = 0;
...
static void checkVaildData(picker_screen_info *psi)
{
    ...
    PickerWidget *wgt = psi->picker;
    SportSettingTypePicker *set = to_set(psi);
    uint32_t sportTime, pickerTime, setTime;

    data_0 = wgt->tool.getNumberByIndex(wgt->index[0], set->min / 60, set->max / 60, 1);
    data_1 = wgt->tool.getNumberByIndex(wgt->index[1], 0, 55, set->interval);

    sportTime  = getSportTime() / 1000 / 60;
    pickerTime = data_0 * 60 + data_1;

    /* 选择器的值小于当前运动时间, 重设时间 */
    if(pickerTime <= sportTime)
    {
        setTime = sportTime / set->interval * set->interval + set->interval;
        wgt->index[0] = wgt->tool.getNumberIndex(setTime / 60, set->min / 60, set->max / 60, set->interval);
        wgt->index[1] = wgt->tool.getNumberIndex(setTime, set->min / 60, set->max / 60, set->interval);
    ...
    {
        wgt->index[1] = wgt->tool.getNumberIndex(set->min, set->min / 60, set->max / 60, set->interval);
        wgt->setValue(wgt, true);
        return;
    }
}
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 兼容protobuf修改 */
    (set->interval == 1) ? (set->interval = 5) : (H_NULL);
    ...
    item_1 = setPicker_ItemNum(psi->picker, &pdata, 1, 0, 55, set->interval, initVal[1]);
    if(item_1 == NULL)
        goto err1;
...
const picker_ops picker_ops_reminder_back_time = {
