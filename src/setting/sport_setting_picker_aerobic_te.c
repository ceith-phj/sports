...
#define to_set(psi)             &psi->ssm.assist.target.useTrainingEffect.aerobicTE
...
static void saveSetting(picker_screen_info *psi)
{
    ...
    rawSet->useTrainingEffect.aerobicTE.onOff = true;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    psi->picker = createPicker(psi->sc, &pdata, PICKER_DOUBLE, _ST(TK_AEROBIC_TE), _ST(subtitle));
...
const picker_ops picker_ops_aerobic_te = {
