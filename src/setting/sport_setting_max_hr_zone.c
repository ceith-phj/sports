...
#define SETTING_SCREEN              "SportSettingMaxHrZoneScreen"               /**< 设置所在screen. */
#define SETTING_LAYER               "SportSettingMaxHrZoneLayer"                /**< 设置所在layer. */

typedef enum _list_item_index {
    INDEX_HR_ZONE_WARM_UP,                                                      /**< 00' 热身放松. */
    INDEX_HR_ZONE_BURNING,                                                      /**< 01' 脂肪燃烧. */
    INDEX_HR_ZONE_AEROBIC,                                                      /**< 02' 心肺强化. */
    INDEX_HR_ZONE_ANEAROBIC,                                                    /**< 03' 耐力强化. */
    INDEX_HR_ZONE_MAX,                                                          /**< 04' 无氧极限. */
...
static void _radioEvent(sport_setting_container *ssc, uint16_t index)
{
    ...
    if(rawSet->hrRange.choice > 6 || rawSet->hrRange.choice < 1) {
        HLOG_I(TAG, "CCCCCCCCCCCC hr range choice = %d", rawSet->hrRange.choice);
...
static void _settingCreate(sport_setting_container *ssc)
{
    ...
    /* 设置列表 */
    setItem_RadioText(lsu, INDEX_HR_ZONE_WARM_UP,   defSet, _GT(TEXT_ID_HR_WARM_UP),   hrInfo.value[0],       hrInfo.value[1],      value[i++], false);
    setItem_RadioText(lsu, INDEX_HR_ZONE_BURNING,   defSet, _GT(TEXT_ID_HR_BURNING),   hrInfo.value[1],       hrInfo.value[2],      value[i++], false);
    setItem_RadioText(lsu, INDEX_HR_ZONE_AEROBIC,   defSet, _GT(TEXT_ID_HR_AEROBIC),   hrInfo.value[2],       hrInfo.value[3],      value[i++], false);
    setItem_RadioText(lsu, INDEX_HR_ZONE_ANEAROBIC, defSet, _GT(TEXT_ID_HR_ANAEROBIC), hrInfo.value[3],       hrInfo.value[4],      value[i++], false);
    setItem_RadioText(lsu, INDEX_HR_ZONE_MAX,       defSet, _GT(TEXT_ID_HR_MAX),       hrInfo.value[4],       hrInfo.value[5],      value[i++], false);
...
const sport_setting_ops setting_ops_max_hr_zone = {
