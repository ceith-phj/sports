...
#define SETTING_SCREEN              "SportSettingEnergyScreen"                  /**< 设置所在screen. */
#define SETTING_LAYER               "SportSettingEnergyLayer"                   /**< 设置所在layer. */
...
static void _textEvent(sport_setting_container *ssc, uint16_t index)
{
    ...
    case INDEX_TARGET_TIME:
        switchPickerScreen(PICKER_ID_REMINDER_ENERGY_TIME, &ssc->rsi.set, ssc->change);
        break;

    case INDEX_TARGET_DISTANCE:
        switchPickerScreen(PICKER_ID_REMINDER_ENERGY_DISTANCE, &ssc->rsi.set, ssc->change);
        break;

    case INDEX_TARGET_CALORIES:
        switchPickerScreen(PICKER_ID_REMINDER_ENERGY_CONSUME, &ssc->rsi.set, ssc->change);
        break;
...
static void _rlwEvent(radio_list_widget *rlw, lw_event_type event, int index, int lastIndex)
{
    ...
    SportSettingUseWorkoutStatistics *rawSet = &ssc->rsi.set.assist.reminder.energy.energy;
    ...
    /* 设置选中值 */
    ssc->rsi.set.assist.reminder.energy.choice = imap[index];
    ssc->change = true;
}
...
static void _settingCreate(sport_setting_container *ssc)
{
    ...
    SportSettingUseWorkoutStatistics *defSet = &ssc->rsi.def.assist.reminder.energy.energy;
    SportSettingUseWorkoutStatistics *rawSet = &ssc->rsi.set.assist.reminder.energy.energy;
    lw_set_utils *lsu = &context->lsu;

    /* 创建列表控件 */
    context->rlw = radioList_Create(SETTING_SCREEN, SETTING_LAYER, _rlwEvent);
    radioList_SetListAttr(context->rlw, LW_ATTR_TITLE, (void *)_ST(TK_ENERGY));
    radioList_SetListAttr(context->rlw, LW_ATTR_PARAM, ssc);
    lwUtils_Init(lsu, itemList, INDEX_MAX);

    /* 设置 - 用时 */
    snprintf(value[i], SZ_SET_BUF, _ST(TK_ARG_PER_HOUR_MIN), (int)(rawSet->time.data)/60, (int)rawSet->time.data % 60);
    setItem_RadioText(lsu, INDEX_TARGET_TIME, defSet->has_time, &defSet->time, &rawSet->time, _ST(TK_BY_TIME), value[i++], &select);

    /* 设置 - 距离 */
    unit = choiceUnitText(_ST(TK_ARG_PER_KM), _ST(TK_ARG_PER_MILE));
    ...
    /* 设置 - 消耗 */
    snprintf(value[i], SZ_SET_BUF, _ST(TK_ARG_PER_KCAL), (int32_t)rawSet->calories.data);
...
const sport_setting_ops setting_ops_energy = {
