...
#define to_set(psi)             &psi->ssm.assist.target.useWorkoutStatistics.time
...
static void saveSetting(picker_screen_info *psi)
{
    ...
    rawSet->useWorkoutStatistics.time.onOff = true;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_DOUBLE, _ST(TK_TARGET_TIME), NULL);
...
const picker_ops picker_ops_target_time = {
