...
#define to_set(psi)             &psi->ssm.assist.reminder.max_cadence
...
static bool checkVaildData(picker_screen_info *psi)
{
    PickerWidget *wgt = psi->picker;
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.min_cadence;
    ...
    {
        set->data = aim->data + 5;
        switchDialogScreen(DLG_ID_MAX_CADENCE, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_FAST_CADENCE), NULL);
...
const picker_ops picker_ops_reminder_max_cadence = {
