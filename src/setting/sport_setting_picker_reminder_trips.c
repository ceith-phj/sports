...
#define to_set(psi)             &psi->ssm.assist.reminder.trip
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_COM_SET_TRIP), NULL);
...
const picker_ops picker_ops_reminder_trips = {
