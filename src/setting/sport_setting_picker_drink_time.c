...
#define ARG_PICKER_0            0, 2, 1                                     /**< 第1列选择器. */
#define to_set(psi)             &psi->ssm.assist.reminder.drinking.drink.time
...
static void _pickerCheckEvent(picker_screen_info *psi)
{
    ...
    /* 组合数据 */
    data_0 = wgt->tool.getNumberByIndex(wgt->index[0], ARG_PICKER_0);
    data_1 = wgt->tool.getNumberByIndex(wgt->index[1], 0, 55, set->interval);
    set->data = (data_0 * 60 + data_1);

    /* 保存设置 */
    psi->ssm.assist.reminder.drinking.choice = 0;
    set->onOff = true;
...
static void checkVaildData(picker_screen_info *psi)
{
    ...
    {
        wgt->index[1] = wgt->tool.getNumberIndex(set->min, 0, 55, set->interval);
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    item_0 = setPicker_ItemNum(psi->picker, &pdata, 0, ARG_PICKER_0, initVal[0]);
    if(item_0 == NULL)
        return;
...
const picker_ops picker_ops_reminder_drink_time = {
