...
#define to_set(psi)             &psi->ssm.assist.target.useWorkoutStatistics.count
...
static void saveSetting(picker_screen_info *psi)
{
    ...
    rawSet->useWorkoutStatistics.count.onOff = true;
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_TARGET_COUNT), NULL);
...
const picker_ops picker_ops_target_count = {
