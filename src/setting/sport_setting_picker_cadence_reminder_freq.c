...
#define to_set(psi)             &psi->ssm.assist.cadenceAssistant.reminderFrequence
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_REMINDER_FREQ), NULL);
...
const picker_ops picker_ops_cadence_reminder_freq = {
