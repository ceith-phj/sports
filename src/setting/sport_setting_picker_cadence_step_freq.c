...
#define to_set(psi)             &psi->ssm.assist.cadenceAssistant.cadence
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_CADENCE), NULL);
...
const picker_ops picker_ops_cadence_step_freq = {
