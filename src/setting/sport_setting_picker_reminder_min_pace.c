...
#define ARG_PICKER_1            5, 55, 5                                     /**< 第1列选择器. */
#define to_set(psi)             &psi->ssm.assist.reminder.low_pace
...
static bool checkPickerData(picker_screen_info *psi)
{
    ...
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.fast_pace;
    PickerWidget *wgt = psi->picker;
    ...
    {
        set->data = aim->data + 5;
        switchDialogScreen(DLG_ID_MIN_PACE, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    psi->picker = createPicker(psi->sc, &pdata, PICKER_DOUBLE, _ST(TK_SLOW_PACE), NULL);
...
const picker_ops picker_ops_reminder_min_pace = {
