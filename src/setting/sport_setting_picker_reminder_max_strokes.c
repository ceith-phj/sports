...
#define to_set(psi)             &psi->ssm.assist.reminder.max_strokes
...
static bool checkPickerData(picker_screen_info *psi)
{
    PickerWidget *wgt = psi->picker;
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.min_strokes;
    float val = wgt->tool.getNumberByIndex(wgt->index[0], set->min, set->max, set->interval);

    /* 比较目标打开并且VALUE冲突时弹窗 */
    if(aim->onOff == true && val <= aim->data)
    {
        set->data = aim->data + 2;
        switchDialogScreen(DLG_ID_MAX_STROKES, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_FAST_STROKES), NULL);
...
const picker_ops picker_ops_reminder_max_strokes = {
