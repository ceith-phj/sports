...
#define ARG_PICKER_1            0, 50, 5                                     /**< 第1列选择器. */
#define to_set(psi)             &psi->ssm.assist.reminder.fast_pace
...
* @return : None
******************************************************************************************************/
static bool checkPickerData(picker_screen_info *psi)
{
    float data_0, data_1, val;
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.low_pace;
    PickerWidget *wgt = psi->picker;

    /* 计算当前配速 */
    data_0 = wgt->tool.getNumberByIndex(wgt->index[0], set->min / 60, set->max / 60, 1);
    data_1 = wgt->tool.getNumberByIndex(wgt->index[1], ARG_PICKER_1);
    val = data_0 * 60 + data_1;

    /* 比较目标打开并且VALUE冲突时弹窗 */
    if(aim->onOff == true && val > aim->data - 5)
    {
        set->data = aim->data - 5;
        switchDialogScreen(DLG_ID_MAX_PACE, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    psi->picker = createPicker(psi->sc, &pdata, PICKER_DOUBLE, _ST(TK_FAST_PACE), NULL);
...
const picker_ops picker_ops_reminder_max_pace = {
