...
#define to_set(psi)             &psi->ssm.assist.reminder.min_strokes
...
static bool checkVaildData(picker_screen_info *psi)
{
    PickerWidget *wgt = psi->picker;
    SportSettingTypePicker *set = to_set(psi);
    SportSettingTypePicker *aim = &psi->ssm.assist.reminder.max_strokes;
    ...
    {
        set->data = aim->data - 2;
        switchDialogScreen(DLG_ID_MIN_STROKES, &psi->ssm, true);
        return false;
    }
...
static void _settingCreate(picker_screen_info *psi)
{
    ...
    /* 创建选择器 */
    psi->picker = createPicker(psi->sc, &pdata, PICKER_SINGLE_WIDE, _ST(TK_SLOW_STROKES), NULL);
...
const picker_ops picker_ops_reminder_min_strokes = {
